    ...
    try:
        ...
        if hf_token:
            ...
            for i, segment in enumerate(diarization.segments[:10]):
                ...
                logger.info(f"  [{i+1}] {segment.start:.1f}-{segment.end:.1f}s ({duration:.1f}s): {segment.speaker}")
            ...
            for speaker, stats in sorted(speaker_stats.items()):
                logger.info(f"  {speaker}: {stats['count']} segments, {stats['total_time']:.1f}s total")
            ...
            logger.info(f"  Processing time: {enhanced_result.processing_time_seconds:.2f}s")
            ...
            for i, segment in enumerate(enhanced_result.speaker_attributed_segments):
                ...
                if text and shown_count < 10:  # Only show segments with text
                    ...
                    logger.info(f"  [{shown_count+1}] {start:.1f}-{end:.1f}s [{speaker}] ({confidence:.2f}): {text}")
                    shown_count += 1
            
            # Count attributed vs unattributed segments
            attributed_count = sum(1 for seg in enhanced_result.speaker_attributed_segments 
                                 if seg.get('speaker') and seg.get('speaker') != 'UNKNOWN')
            total_with_text = sum(1 for seg in enhanced_result.speaker_attributed_segments 
                                if seg.get('text', '').strip())
            
            logger.info(f"\nAttribution statistics:")
            logger.info(f"  Segments with text: {total_with_text}")
            logger.info(f"  Segments with speaker attribution: {attributed_count}")
            logger.info(f"  Attribution rate: {attributed_count/total_with_text*100:.1f}%" if total_with_text > 0 else "N/A")
            
        else:
            logger.warning("\n" + "=" * 60)
            logger.warning("Step 2: Skipping Speaker Diarization")
            logger.warning("=" * 60)
            logger.warning("No Hugging Face token found. Set HUGGINGFACE_TOKEN environment variable")
