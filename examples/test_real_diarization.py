...
def test_real_diarization():
    ...
    try:
        ...
        for speaker_id in sorted(speakers):
            ...
            print(
                f"   {speaker_id}: {len(speaker_segments)} segments, {total_duration:.1f}s total"
            )
        ...
        for i, segment in enumerate(result.segments[:10]):
            print(
                f"   [{segment.start:6.1f}s - {segment.end:6.1f}s] {segment.speaker} (conf: {segment.confidence:.3f})"
            )
