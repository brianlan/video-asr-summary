...
def print_integrated_segments(segments: list):
    ...
    for i, segment in enumerate(segments, 1):
        ...
        speaker_info = f"{speaker} (conf: {confidence:.2f})" if speaker else "No speaker"
...
def demo_with_audio_file(audio_path: Path, hf_token: Optional[str] = None):
    ...
    try:
        ...
        print(f"✅ Whisper completed in {transcription_result.processing_time_seconds:.2f}s")
        ...
        if hf_token:
            ...
            try:
                ...
                print(f"✅ Diarization completed in {diarization_result.processing_time_seconds:.2f}s")
                ...
                print_diarization_segments(diarization_result.segments[:10])  # Show first 10
                
                # Step 3: Integration
                print("\n🔗 Integrating ASR and diarization...")
                integrator = SegmentBasedIntegrator(overlap_threshold=0.3)
                integrated_result = integrator.integrate(transcription_result, diarization_result)
                
                print(f"✅ Integration completed in {integrated_result.processing_time_seconds:.2f}s")
                
                # Display integrated segments
                print_integrated_segments(integrated_result.speaker_attributed_segments[:10])
                
                # Show alignment analysis
                print_separator("ALIGNMENT ANALYSIS")
                analyze_segment_alignment(
                    transcription_result.segments[:5], 
                    diarization_result.segments[:5]
                )
                
            except Exception as e:
                print(f"❌ Diarization failed: {e}")
                print("💡 This might be due to missing Hugging Face token or model license issues")
        else:
