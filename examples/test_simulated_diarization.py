...
from video_asr_summary.core import SpeakerSegment, DiarizationResult
...
def test_asr_with_simulated_diarization(audio_path: str):
    ...
    try:
        ...
        for i, segment in enumerate(diarization.segments[:10]):
            ...
            logger.info(f"  [{i+1}] {segment.start:.1f}-{segment.end:.1f}s ({duration:.1f}s): {segment.speaker} (conf: {segment.confidence:.2f})")
        ...
        for segment in enhanced_result.speaker_attributed_segments:
            ...
            if text and shown_count < 10:
                ...
                logger.info(f"  [{shown_count+1}] {start:.1f}-{end:.1f}s [{speaker}] ({confidence:.2f}): {display_text}")
                shown_count += 1
        ...
        if total_with_text > 0:
            logger.info(f"  Attribution rate: {attributed_count/total_with_text*100:.1f}%")
        ...
        for speaker in sorted(speaker_stats.keys()):
            ...
            logger.info(f"  {speaker}: {stats['count']} segments, {stats['chars']} characters")
