...
def demo_video_to_text_pipeline():
    ...
    print(
        f"     Processing time: {simulated_transcription.processing_time_seconds:.1f} seconds"
    )
    ...
    for i, segment in enumerate(simulated_transcription.segments):
        ...
        print(
            f"     {i+1}. [{start:5.1f}s - {end:5.1f}s] {text} (conf: {confidence:.3f})"
        )
    print()
    ...
    print("   ```python")
    print(
        "   def process_video_to_text(video_path: Path, language: str = 'en') -> TranscriptionResult:"
    )
    print("       # Extract audio")
