...
def demo_diarization_pipeline(video_path: str, hf_token: Optional[str] = None):
    ...
    try:
        ...
        diarization_processor = PyannoteAudioProcessor(
            auth_token=hf_token, device="auto"
        )
        ...
        for i, segment in enumerate(
            enhanced_result.speaker_attributed_segments[:15]
        ):  # Show first 15
            ...
            if text:  # Only show segments with text
                logger.info(
                    f"{start:6.1f}-{end:5.1f}s [{speaker}] ({confidence:.2f}): {text}"
                )

        if len(enhanced_result.speaker_attributed_segments) > 15:
            logger.info(
                f"... and {len(enhanced_result.speaker_attributed_segments) - 15} more segments"
            )
