...
def test_imports():
    ...
    try:
        ...
        from video_asr_summary.diarization.pyannote_processor import (
            PyannoteAudioProcessor,
        )
        ...
        speaker_segment = SpeakerSegment(
            start=0.0, end=10.0, speaker="SPEAKER_00", confidence=0.95
        )
        print(f"✅ SpeakerSegment created: {speaker_segment}")

        diarization_result = DiarizationResult(
            segments=[speaker_segment], num_speakers=1, processing_time_seconds=1.5
        )
        print(
            f"✅ DiarizationResult created: {diarization_result.num_speakers} speakers"
        )

        # Test integrator creation
        integrator = SegmentBasedIntegrator(overlap_threshold=0.5)
        print(
            f"✅ SegmentBasedIntegrator created with threshold: {integrator.overlap_threshold}"
        )
...
def test_pyannote_availability():
    ...
    try:
        ...
        print(
            "⚠️  pyannote.audio is not installed (install with: pip install pyannote.audio)"
        )
        return False
