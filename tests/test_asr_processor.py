...
class TestWhisperProcessor:
    ...
    def test_transcribe_with_language_hint(self, mock_exists, mock_mlx_whisper):
        ...
        mock_result = {
            'text': '这是一个测试转录。',
            'segments': [{'start': 0.0, 'end': 2.0, 'text': '这是一个测试转录。', 'avg_logprob': -0.4, 'no_speech_prob': 0.1}],
            'language': 'zh'
        }
        mock_mlx_whisper.transcribe.return_value = mock_result
    ...
    def test_calculate_confidence_score(self, mock_exists, mock_mlx_whisper):
        ...
        mock_result = {
            ...
            'segments': [
                {'start': 0.0, 'end': 1.0, 'text': 'Test', 'avg_logprob': -0.2, 'no_speech_prob': 0.1},
                {'start': 1.0, 'end': 2.0, 'text': ' transcription.', 'avg_logprob': -0.4, 'no_speech_prob': 0.05}
            ],
            'language': 'en'
        }
        mock_mlx_whisper.transcribe.return_value = mock_result
