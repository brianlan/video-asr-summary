...
from ..core import DiarizationResult, SpeakerDiarizationProcessor, SpeakerSegment
...
class PyannoteAudioProcessor(SpeakerDiarizationProcessor):
            ...
            try:
                ...
                if self.device in ["cuda", "mps"]:
                    try:
                        ...
                        logger.warning(
                            "PyTorch not available for GPU acceleration, using CPU"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Failed to move pipeline to {self.device}: {e}, using CPU"
                        )
                ...
                raise RuntimeError(
                    f"Failed to load pyannote.audio pipeline: {e}"
                ) from e
    ...
    def diarize(
        ...
        try:
            ...
            self._load_pipeline()
            assert self._pipeline is not None, "Pipeline should be loaded"
